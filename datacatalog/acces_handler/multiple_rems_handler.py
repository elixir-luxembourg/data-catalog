...
class MultipleRemsAccessHandler(AccessHandler):
    ...
    def __init__(
        ...
        self.rems_connectors = {
            instance_id: RemsConnector(
                api_username=api_username,
                api_key=app.config["REMS_INSTANCES"][instance_id]["REMS_API_KEY"],
                host=app.config["REMS_INSTANCES"][instance_id]["REMS_URL"],
                workflow_id=app.config["REMS_INSTANCES"][instance_id][
                    "REMS_WORKFLOW_ID"
                ],
                verify_ssl=verify_ssl,
                admin_user=app.config["REMS_INSTANCES"][instance_id]["REMS_API_USER"],
            )
        ...
        if user.is_authenticated:
            ...
            for _, rems_connector in self.rems_connectors.items():
                rems_connector.create_user(api_username, user.displayname, user.email)
        super().__init__(user)
    ...
    def has_access(self, dataset):
        ...
        if not dataset.e2e:
            ...
            return False
        applications = self.get_rems_connector(dataset).applications(
            f'resource:"{dataset.id}"'
        )
        has_submitted = False
        if not applications:
            return False
    ...
    def my_applications(self):
        ...
        applications = []
        for rems_connector in self.rems_connectors.values():
    ...
    def apply(self, dataset, form):
        ...
        rems_connector = self.get_rems_connector(dataset)

        try:
            catalogue_item = rems_connector.get_catalogue_item(dataset)
        ...
        for field in rems_form.fields:
            ...
            field_values[rems_field_id] = wtf_field.transform_value(
                flask_form_value, rems_connector, application_id
            )
